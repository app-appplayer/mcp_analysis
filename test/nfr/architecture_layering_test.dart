...
///   feat/transform)  ←  domain tier 1 (feat/domain)
///
...
void main() {
    ...
    test('compute layer (feat/function, feat/transform) imports no domain', () {
      for (final f in [
        ...
        expect(src.contains(RegExp(r"import\s+'[^']*feat/domain")), isFalse,
            reason: '${f.path}: compute must not depend on domain tier 1 '
                '(dependency direction is domain → compute)');
      }
    });

    test('domain tier 1 reaches only compute helpers and ports', () {
      for (final f in dartFilesUnder('lib/src/feat/domain')) {
        ...
        for (final imp in imports) {
          ...
          expect(allowed, isTrue,
              reason: '${f.path} imports "$imp" — domain tier 1 may use only '
  ...
  group('package purity (the tree-shaking / tier-1 rules)', () {
    ...
    test('runtime dependencies stay the frozen minimal set', () {
      ...
      final depsSection =
          pubspec.split('dependencies:')[1].split('dev_dependencies:')[0];
      final deps = RegExp(r'^\s{2}(\w+):', multiLine: true)
          .allMatches(depsSection)
          .map((m) => m.group(1)!)
          .toSet();
      expect(deps, {'mcp_bundle', 'collection', 'meta'},
          reason: 'adding a runtime dependency breaks the tier-1 embedding '
              'argument (pub resolves deps even when code is tree-shaken) — '
              'dependency-carrying features are tier-2 external packs');
    });
  });
}
